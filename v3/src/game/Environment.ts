...
export class Environment {
  ...
  constructor(scene: THREE.Scene) {
    ...
    // The defensive ridge: a ragged double ring of crags with gaps —
    // the line everything has to funnel through.
    for (let i = 0; i < 64; i++) {
      const angle = (i / 64) * Math.PI * 2 + (rand() - 0.5) * 0.04;
      ...
      for (const row of [-0.6, 0.6]) {
        ...
        crag.position.set(Math.cos(angle) * r, s * 0.45, Math.sin(angle) * r);
    ...
    for (const g of GAP_ANGLES) {
      for (const side of [-1, 1]) {
        ...
        spire.position.set(Math.cos(a) * RIDGE_RADIUS, h / 2 - 0.2, Math.sin(a) * RIDGE_RADIUS);
        scene.add(spire);
      }
    }
