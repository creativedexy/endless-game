...
import { clampToRidge } from './ridge';
...
export class GameManager {
  ...
  constructor(container: HTMLElement) {
    ...
    // Build pads: a pair guarding the inside of each ridge gap (the
    // defence line), plus a small "village" cluster near the wreck.
    for (const gap of GAP_ANGLES) {
      for (const side of [-1, 1]) {
        const a = gap + side * 0.22;
        this.pads.push(new BuildPad(this.scene, Math.cos(a) * 11, Math.sin(a) * 11));
      }
    }
    // Kept clear of the player spawn so nothing auto-builds at second 0.
    for (const deg of [90, 200, 320]) {
      const a = (deg * Math.PI) / 180;
      this.pads.push(new BuildPad(this.scene, Math.cos(a) * 6.5, Math.sin(a) * 6.5));
    }
  ...
  private spawnAmbientPickup() {
    for (let attempt = 0; attempt < 12; attempt++) {
      ...
      if (Math.abs(r - RIDGE_RADIUS) < RIDGE_BAND + 0.6) continue;
