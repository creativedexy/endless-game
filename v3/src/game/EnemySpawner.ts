...
export class EnemySpawner {
  ...
  update(dt: number, game: GameManager) {
    ...
    // Waves pour toward a random ridge gap, so attacks read as pushes
    // against one front rather than an even trickle.
    const gap = GAP_ANGLES[Math.floor(Math.random() * GAP_ANGLES.length)];
    const baseAngle = gap + (Math.random() - 0.5) * 1.1;
