...
// `firePressed` / `dashPressed` / `restartPressed` / `menuKey` are
// edge-triggered and consumed once per frame by the GameManager.
...
export interface InputState {
  ...
  menuKey: number; // 0 = none, 1..4 = build-menu slot pressed this frame
}
...
/** WASD / arrows + Space or mouse to shoot + Shift dash + R restart + 1-4 build. */
