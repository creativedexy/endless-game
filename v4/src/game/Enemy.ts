...
export class Enemy {
  ...
  update(dt: number, game: GameManager) {
    ...
    // Ridge navigation: crossing the defence arc in either direction means
    // walking to the nearest gap first instead of face-planting into crags.
    let moveTarget = this.targetPos;
    let atTarget = true;
    const myR = distFromBase(this.position.x, this.position.z);
    const targetR = distFromBase(this.targetPos.x, this.targetPos.z);
    const crossing =
      (myR > RIDGE_RADIUS + 0.4 && targetR < RIDGE_RADIUS - 0.4) ||
      (myR < RIDGE_RADIUS - 0.4 && targetR > RIDGE_RADIUS + 0.4);
    if (crossing) {
      const myAngle = angleFromBase(this.position.x, this.position.z);
      if (inArc(myAngle) ? !inGap(myAngle) : true) {
        // Outside the arc span there are cliffs anyway; aim for a gap.
