...
export class EnemySpawner {
  ...
  update(dt: number, game: GameManager) {
    ...
    // Waves come in from the wild edges (north and west) as one group, so
    // attacks read as pushes against a front rather than an even trickle.
    let px: number;
    let pz: number;
    if (Math.random() < 0.5) {
      px = MAP_MIN_X + Math.random() * (MAP_MAX_X - MAP_MIN_X);
      pz = MAP_MIN_Z - 2; // north edge
    } else {
      px = MAP_MIN_X - 2; // west edge
