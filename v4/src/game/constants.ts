...
// Ore deposits out in the wilds — the only places a Mine can be built.
...
// What the blueprint selector offers; mine/village are node-bound instead.
export const SELECTOR_KINDS: StructureKind[] = [
  'turret',
  'wall',
  'extractor',
  'forge',
...
/**
 * Every build spot on the map. The GameManager spawns pads from this and
 * the Environment keeps rocks and spires clear of them. Defence pads pair
 * up inside each ridge gap plus one forward post outside it; the base
 * cluster sits by the wreck; village plots hug the cliffs; mine sites sit
 * on the ore deposits.
 */
export const PAD_LAYOUT: Array<{ x: number; z: number; kind: StructureKind | null }> = (() => {
  const pads: Array<{ x: number; z: number; kind: StructureKind | null }> = [];
  for (const gap of GAP_ANGLES) {
    for (const side of [-1, 1]) {
      const a = gap + side * 0.2;
      pads.push({ x: BASE_X + Math.cos(a) * 12.5, z: BASE_Z + Math.sin(a) * 12.5, kind: null });
    }
    pads.push({
  ...
  for (const deg of [175, 225, 275]) {
    const a = (deg * Math.PI) / 180;
    pads.push({ x: BASE_X + Math.cos(a) * 7, z: BASE_Z + Math.sin(a) * 7, kind: null });
  }
