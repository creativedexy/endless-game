...
export class BuildPad {
  ...
  constructor(
    ...
    x: number,
    z: number,
    readonly fixedKind: StructureKind | null = null,
  ) {
    this.position = new THREE.Vector3(x, 0, z);
    this.ringColor =
      fixedKind === 'mine' ? COLORS.salvage : fixedKind === 'village' ? COLORS.beacon : COLORS.padRing;
  ...
  /** What this pad will build: its bound node type, or the armed blueprint. */
  buildKind(selected: StructureKind): StructureKind {
    return this.fixedKind ?? selected;
  }

  /** Convenience for cost/label lookups. */
  defFor(selected: StructureKind) {
    return STRUCTURE_DEFS[this.buildKind(selected)];
  }
