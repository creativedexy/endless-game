...
export class GameManager {
  ...
  private blueprintDefs = SELECTOR_KINDS.map((k) => STRUCTURE_DEFS[k]);
  ...
  constructor(container: HTMLElement) {
    ...
    // Build pads — defence line, base cluster, village plots and mine
    // sites all come from the shared layout (kept clear of the player
    // spawn so nothing auto-builds at second 0).
    for (const spot of PAD_LAYOUT) {
      this.pads.push(new BuildPad(this.scene, spot.x, spot.z, spot.kind));
    }
  ...
  private updateBuildContext(dt: number) {
    ...
    for (const pad of this.pads) {
      ...
      // Node pads glow for their own building's cost, not the blueprint's.
      pad.setState(d < INTERACT_RANGE, this.canAfford(pad.defFor(selectedDef.kind).buildCost));
      if (d < nearestDist) {
        nearestDist = d;
        nearestPad = pad;
      }
    ...
    if (pad) {
      ...
      if (pad.isEmpty) {
        // Node pads (deposits, village plots) build their bound structure;
        // ordinary pads build whatever blueprint is armed.
        const def = pad.defFor(selectedDef.kind);
