...
- **No head, no shortcut** — every worm is one fused, undifferentiated chain from end to end; there's no special segment to snipe for an instant kill, and the body renders as a single continuous tube rather than a string of separate beads. You have to clear the whole body, and it gets measurably tougher the deeper you cut in, so stalling partway through is a losing bet. Segment HP is far higher across the board than earlier builds.
- **Rectilinear raster routes** — the route runs straight across to one wall, turns a sharp corner, drops straight down, turns another sharp corner, and runs straight back across to the other wall — repeating in a boustrophedon sweep all the way down the screen, no diagonals or curves. Row count and spacing vary by stage, so you're constantly sliding the turret to stay under the body as it sweeps edge-to-edge.
...
- **Capsules are rare and punish hesitation** — sealed mutation capsules are now scarce (roughly one per 16–22 segments, one per boss). Crack one to 50% to reveal its reward category and it starts *ripening*: every second it survives un-destroyed feeds the worm carrying it +1.5% speed (capped per capsule, stacking across several), visibly glowing hotter the longer it's ignored — so "I'll get it later" is a real, compounding cost, not a suggestion. Each of the nine reward pools has its own hand-drawn icon glyph. Trap capsules permanently harden the worm and chip the core on trigger instead of spawning anything.
- **Every surviving segment counts** — anything that reaches the breach deals damage per surviving segment (scaled up further by any capsules you let ripen), so partial kills still hurt.
- **Two threads, two lifespans** — **Abilities** (six weapons — Pulse Needle, Cleanse Orb, Antibody Drones, Ion Arc, Enzyme Cloud, Compression Wave — plus Spore Wing, Husk Roller and Cinder Gland; install, level and evolve them) are permanent for the whole run. **Boosts** (a short, practical pool — multishot, damage%, fire rate, crit, pierce, healing, core HP) are wiped back to zero at the start of every stage, so you re-earn your build's power stage by stage instead of snowballing forever. Max a weapon and crack an evolution capsule to transform it (Needle Constellation, Storm Lattice, Seismic Collapse…).
- **Worm roster & bosses** — runners, armoured worms, regenerators, a Hive Worm that hardens itself further with every segment you kill, phase worms, an Ironclad Worm that fuses and hardens at the halfway mark, plus multi-phase segmented bosses (in its final phase, the Crowned Devourer sheds its trailing body, absorbs the mass into its foremost segment, and makes a tougher, faster run for the breach).
